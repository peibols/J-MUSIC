...
symmetric_cumulant_name_list = ['SC_32', 'SC_42', 'SC_52', 'SC_53']
...
def calcualte_vn_2(vn_data_array, vn_data_array_ref, eta_gap_flag):
    """
    ...
    dN_ref = dN_ref.reshape(len(dN_ref), 1)
    Qn_ref = dN_ref*vn_data_array_ref[:, 1:]
    if eta_gap_flag==0 :
       corr = 1./(dN*(dN - 1.))*(Qn_array*conj(Qn_array) - dN)#remove self correlation
    else:
       corr = Qn_array*conj(Qn_ref)/(dN*dN_ref)
...
##SM 3/12/18
    Q7 = dN*vn_data_array[:, 7]
    Q8 = dN*vn_data_array[:, 8]
##
...
###new SM 3/12/18
    Q_52 = ((abs(Q2)**2.)*(abs(Q5)**2.) - 2.*real(Q7*conj(Q2)*conj(Q5))
        - 2.*real(Q5*conj(Q2)*conj(Q3)) + abs(Q7)**2. + abs(Q3)**2.
        - (dN - 4.)*(abs(Q2)**2. + abs(Q5)**2.) + dN*(dN - 6.)
    )
    ## need Q8 for this##
    Q_53 = ((abs(Q3)**2.)*(abs(Q5)**2.) - 2.*real(Q8*conj(Q3)*conj(Q5))
        - 2.*real(Q5*conj(Q2)*conj(Q3)) + abs(Q8)**2. + abs(Q2)**2.
        - (dN - 4.)*(abs(Q3)**2. + abs(Q5)**2.) + dN*(dN - 6.)
    )
###
    ...
    SC52_array = zeros(nev)
    SC53_array = zeros(nev)
    ...
    for iev in range(nev):
        ...
        SC42_array[iev] = (mean(Q_42[array_idx])/mean(N4_weight[array_idx])
                           - (mean(Q4_2[array_idx])*mean(Q2_2[array_idx]))
                             /(mean(N2_weight[array_idx])**2.))
      # SC(5,2)
        SC52_array[iev] = (mean(Q_52[array_idx])/mean(N4_weight[array_idx])
                           - (mean(Q5_2[array_idx])*mean(Q2_2[array_idx]))
                             /(mean(N2_weight[array_idx])**2.))
      # SC(5,3)
        SC53_array[iev] = (mean(Q_53[array_idx])/mean(N4_weight[array_idx])
                           - (mean(Q5_2[array_idx])*mean(Q3_2[array_idx]))
                             /(mean(N2_weight[array_idx])**2.))
    ...
    SC52_mean = mean(SC52_array)
    SC52_err = sqrt((nev - 1.)/nev*sum((SC52_array - SC52_mean)**2.))

    SC53_mean = mean(SC53_array)
    SC53_err = sqrt((nev - 1.)/nev*sum((SC53_array - SC53_mean)**2.))


    results = [SC32_mean, SC32_err, SC42_mean, SC42_err, SC52_mean, SC52_err, SC53_mean, SC53_err]
    return(results)
...
for ipart, particle_id in enumerate(particle_list):
    ...
    file_name_ref = 'particle_9999_vndata_diff_eta_1.5_2.5.dat'
    ...
    for ifolder in range(nev):
        ...
        temp_vn_array_ref = calcualte_inte_vn(0.2, 3.0, temp_data_ref)
        vn_alice_array.append(temp_vn_array)
        vn_alice_ref_array.append(temp_vn_array_ref)
        ...
        temp_vn_array = calcualte_inte_vn(0.5, 2.0, temp_data) ##was 3.0
        ...
        temp_vn_diff_real, temp_vn_diff_imag, temp_dn_diff, temp_vn2 = (
                        calculate_diff_vn_single_event(0.5, 2.0, temp_data,
                                                       temp_data_ref)) ## was 3.0
    ...
    # calcualte vn{2}
    eta_gap = 1
    vn_phenix_2, vn_phenix_2_err = calcualte_vn_2(vn_phenix_array, vn_phenix_array, 0)
    vn_star_2, vn_star_2_err = calcualte_vn_2(vn_star_array, vn_star_array, 0)
    vn_alice_2, vn_alice_2_err = calcualte_vn_2(vn_alice_array, vn_alice_ref_array, eta_gap)
    vn_cms_2, vn_cms_2_err = calcualte_vn_2(vn_cms_array, vn_cms_array, 0)
    vn_atlas_2, vn_atlas_2_err = calcualte_vn_2(vn_atlas_array, vn_atlas_array, 0)
    ...
    if (particle_id == '9999'):
        ...
        #Normalized SC
#        print((vn_alice_2[1]*vn_alice_2[2])**2, (vn_alice_2[2]*vn_alice_2[4])**2)
        NSC_32 = SC_alice[0]/(vn_alice_2[1]*vn_alice_2[2])**2
        NSC_42 = SC_alice[2]/(vn_alice_2[1]*vn_alice_2[3])**2
        NSC_52 = SC_alice[4]/(vn_alice_2[1]*vn_alice_2[4])**2
        NSC_53 = SC_alice[6]/(vn_alice_2[2]*vn_alice_2[4])**2
        print(SC_alice[0], NSC_32, vn_alice_2[0], vn_alice_2[1], vn_alice_2[2])

	#Normalized SC error
        NSC_32_err = abs(NSC_32)*sqrt((SC_alice[1]/SC_alice[0])**2 + (vn_alice_2_err[1]/vn_alice_2[1])**2+(vn_alice_2_err[2]/vn_alice_2[2])**2)
        NSC_42_err = abs(NSC_42)*sqrt((SC_alice[3]/SC_alice[2])**2 + (vn_alice_2_err[1]/vn_alice_2[1])**2+(vn_alice_2_err[3]/vn_alice_2[3])**2)
        NSC_52_err = abs(NSC_52)*sqrt((SC_alice[5]/SC_alice[4])**2 + (vn_alice_2_err[1]/vn_alice_2[1])**2+(vn_alice_2_err[4]/vn_alice_2[4])**2)
        NSC_53_err = abs(NSC_53)*sqrt((SC_alice[7]/SC_alice[6])**2 + (vn_alice_2_err[2]/vn_alice_2[2])**2+(vn_alice_2_err[4]/vn_alice_2[4])**2)
    ...
    if (particle_id =='9999'):
        ...
        # output normalized symmetric cumulants for ALICE pt cut
        output_filename = ("normalized_symmetric_cumulant_ALICE.dat")
        f = open(output_filename, 'w')
        f.write("# type  value  stat. err\n")
        f.write("%s  %.10e  %.10e  \n" % (symmetric_cumulant_name_list[0], NSC_32, NSC_32_err))
        f.write("%s  %.10e  %.10e  \n" % (symmetric_cumulant_name_list[1], NSC_42, NSC_42_err))
        f.write("%s  %.10e  %.10e  \n" % (symmetric_cumulant_name_list[2], NSC_52, NSC_52_err))
        f.write("%s  %.10e  %.10e  \n" % (symmetric_cumulant_name_list[3], NSC_53, NSC_53_err))
        f.close()
        shutil.move(output_filename, avg_folder)
