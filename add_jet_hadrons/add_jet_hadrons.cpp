...
int main() {
    ...
    void append_jet_hadrons_to_OSCAR(std::vector<hadron_info> &jet_hadrons, int num_of_jet_hadrons);
    ...
    append_jet_hadrons_to_OSCAR(jet_hadrons, num_of_jet_hadrons);
...
void append_jet_hadrons_to_OSCAR(std::vector<hadron_info> &jet_hadrons, int num_of_jet_hadrons) {
	...
	int total_number_of_particles = num_of_particles + num_of_jet_hadrons;
        oscar << std::setw(10) << event_num << "  "
              ...
              << std::setw(8) << 0.0 << "  " << std::setw(8) << 0.0 << std::endl;
	for (int idx = 0; idx < num_of_particles; idx++) {
            std::getline(softhad, line);
            oscar << line << std::endl;
	}
	for (int ipart = 0; ipart < num_of_jet_hadrons; ipart++) {
            int partnum = ipart + num_of_particles + 1;
            oscar << std::setw(10) << partnum << "  "
                  << std::setw(10) << jet_hadrons[ipart].part_id
                  << "  ";
            sprintf(line_buffer,
                    "%24.16e  %24.16e  %24.16e  %24.16e  %24.16e  %24.16e  %24.16e  %24.16e  %24.16e",
                     jet_hadrons[ipart].px,
                     jet_hadrons[ipart].py,
                     jet_hadrons[ipart].pz,
                     jet_hadrons[ipart].energy,
                     jet_hadrons[ipart].mass,
                     jet_hadrons[ipart].x,
                     jet_hadrons[ipart].y,
                     jet_hadrons[ipart].z,
                     jet_hadrons[ipart].t);
            oscar << line_buffer << std::endl;
	}
    }
    softhad.close();
    oscar.close();
