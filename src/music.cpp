...
//! This function initialize jets
int MUSIC::initialize_jets() {
   ...
   if (DATA.jet_medium_response_flag == 3) {
       ...
       //std::cout << " Parton List Size = " << jets->parton_list().size() << std::endl;
       flag_jets_initialized = 1;
   }    
   return(0);
}
...
int MUSIC::run_hydro() {
    ...
    evolve = new Evolve(eos, DATA, hydro_source_terms, *jets);
