...
void Freeze::compute_thermal_particle_spectra_and_vn(InitData* DATA) {
    ...
    if (particleMax < 21) {
        ...
        //Total 4-momentum
	OutputIntegrated4Momentum(DATA, 0, 0.01, 3.0);
    
        //Medium response Distributions
	OutputIntegratedMedResDists(DATA, 0, 0.01, 3.0);
	OutputIntegratedMedResDists(DATA, 0, 0.01, 0.5);
    }
}
...
// Output total 4-momentum 
void Freeze::OutputIntegrated4Momentum(
        InitData *DATA, int yflag, double pT_min, double pT_max) {
    
    double eta_min = DATA->dNdy_eta_min;
    double eta_max = DATA->dNdy_eta_max;
    
    music_message << "Calculating integrated 4-momentum between "
         << pT_min << " < p_T < " << pT_max << " and "
         << eta_min << " < eta < " << eta_max << "...";
    music_message.flush("info");
    ...
    // Set output file name
    stringstream fname;
    fname << "./outputs/Total_4mom_" << pT_min << "_" << pT_max 
          << "_eta_" << eta_min << "_" << eta_max << ".dat";
    ...
    // header of the files
    outfile << "Px Py Pz E" << endl; 
    
    double total_mom[4] = {0};

    for (int i = 1; i < particleMax; i++) {
        int number = particleList[i].number;
	double *total_mom_temp = new double [4];
	get_total_momentum(DATA, number, pT_min, pT_max, 0, eta_min, eta_max, total_mom_temp);
	for (unsigned a=0; a<4; a++) total_mom[a]+=total_mom_temp[a];
	delete [] total_mom_temp;
    }

    outfile << scientific << setprecision(8) << total_mom[0] << " " << total_mom[1] << " " << total_mom[2] << " " << total_mom[3] << endl;
    outfile.close();

}

// Output Medium Response Dists wrt y=0, phi=0 
void Freeze::OutputIntegratedMedResDists(
        InitData *DATA, int yflag, double pT_min, double pT_max) {
    
    double eta_min = DATA->dNdy_eta_min;
    double eta_max = DATA->dNdy_eta_max;
    
    music_message << "Calculating medium response dists between "
         << pT_min << " < p_T < " << pT_max << " and "
         << eta_min << " < eta < " << eta_max << "...";
    music_message.flush("info");
    ...
    //Compute Dists for pions, kaons, protons 
    for (int k = 0; k < pid_list_length; k++) {
        int number = pid_list[k];
        int id = partid[MHALF+number];
        if (id < particleMax) {
   	  //Pt Dist 
	  get_dNdpt_dist(DATA, number, pT_min, pT_max, 0, eta_min, eta_max);
   	  //Eta Dist 
	  get_dNdeta_dist(DATA, number, pT_min, pT_max, 0, eta_min, eta_max);
   	  //Phi Dist 
	  get_dNdphi_dist(DATA, number, pT_min, pT_max, 0, eta_min, eta_max);
	}
    }
...
// Compute total 4-momentum of all particles
void Freeze::get_total_momentum(InitData *DATA, int number,
				  double minpt, double maxpt,
				  int yflag, double minrap, double maxrap,
				  double total_mom[4]) {
    ...
    double ylist[neta] = {0};
    double etalist[neta] = {0};
    double philist[nphi+1] = {0};

    double pmudndydptdphi[nphi+1] = {0};
    double pmudndydpt[neta] = {0};
    double pmudndpt[npt] = {0};

    //TEST
    //cout << "minpt= " << minpt << " maxpt= " << maxpt << " npt " << npt << endl;
    //for (int ipt=0; ipt<npt; ipt++) {
      //  cout << " particleList[j].pt[ipt]= " << particleList[j].pt[ipt] << endl;
    //}
	...
	total_mom[mu]=0.;   
        ...
        for (int ipt = 0; ipt < npt; ipt++) {
	        ...
	        double rap_local = particleList[j].y[iy];

                double eta_local, y_local;
                if (DATA->pseudofreeze == 1) {
                  eta_local = rap_local;
                  etalist[iy] = eta_local;
                  y_local = Rap(eta_local, pt, m);   // get rapidity
                  ylist[iy] = y_local;   // get rapidity
                } else {
                  y_local = rap_local;
                  ylist[iy] = y_local;
                  // get Pseudo-rapidity
                  eta_local = PseudoRap(y_local, pt, m);
                  etalist[iy] = eta_local;   // get Pseudo-rapidity
                }

	        for (int iphi = 0; iphi < nphi+1; iphi++) {
	            double phi = iphi*2*PI/nphi;
		    double mom_comp=0.;
		    if (int(mu)==0) mom_comp=pt*cos(phi);
		    else if (int(mu)==1) mom_comp=pt*sin(phi);
		    else if (int(mu)==2) mom_comp=pt*sinh(eta_local);
		    else mom_comp=sqrt(m*m+pt*pt*cosh(eta_local)*cosh(eta_local));
		    philist[iphi] = phi;
	            pmudndydptdphi[iphi]=mom_comp*dydeta(eta_local, pt, m)*pt*particleList[j].dNdydptdphi[iy][ipt][iphi];
	            //pmudndydptdphi[iphi]=cos(phi);
		}

		//TEST
		//cout << " minphi= " << minphi << " maxphi " << maxphi << " nphi= "<< nphi << endl;
		//for (int iphi=0; iphi<nphi+1; iphi++) {
		  //  cout << " philist[iphu]= " << philist[iphi] << endl;
		//}
	   
	        //cout << " Phi Start GSL Integral " << endl;	
	        gsl_interp_accel *phiacc = gsl_interp_accel_alloc ();
	        gsl_spline *phispline = gsl_spline_alloc (gsl_interp_linear, nphi+1);
	        gsl_spline_init (phispline, philist , pmudndydptdphi , nphi+1);

	        //cout << " Phi GSL Integral Initialized " << endl;	
	        //pmudndydpt[ipt]=gsl_spline_eval_integ(phispline, philist[0], philist[nphi], phiacc);
    		pmudndydpt[iy]=gsl_spline_eval_integ(phispline, minphi, maxphi, phiacc);
		//cout << " check integral of cos(phi)= " << gsl_spline_eval_integ(phispline, philist[0], philist[nphi], phiacc);
	        gsl_spline_free (phispline);
                gsl_interp_accel_free (phiacc);
    
		//cout << " Phi GSL Integral OK " << endl;	
            }
        
            gsl_interp_accel *rapacc = gsl_interp_accel_alloc ();
            gsl_spline *rapspline = gsl_spline_alloc (gsl_interp_linear, neta);
            gsl_spline_init (rapspline, etalist, pmudndydpt , neta);
	
	    pmudndpt[ipt]=gsl_spline_eval_integ(rapspline, minrap, maxrap, rapacc);    
            gsl_spline_free (rapspline);
            gsl_interp_accel_free (rapacc);
	    
        }

	gsl_interp_accel *ptacc = gsl_interp_accel_alloc ();
	gsl_spline *ptspline = gsl_spline_alloc (gsl_interp_linear, npt);
	gsl_spline_init (ptspline, particleList[j].pt , pmudndpt , npt);
	    
        total_mom[int(mu)]=gsl_spline_eval_integ(ptspline, minpt, maxpt, ptacc);
...
// Compute dN/dpt Med. Res. Dist.
void Freeze::get_dNdpt_dist(InitData *DATA, int number,
				  double minpt, double maxpt,
				  int yflag, double minrap, double maxrap) {
    ...
    double dndydptdphi[nphi+1] = {0};
    double dndydpt[neta] = {0};
    double dndpt[npt] = {0};

    // Set output file name
    stringstream fname;
    fname << "./outputs/MedRes_" << number << "_dNdpt_" << minpt << "_" << maxpt
          << "_eta_" << minrap << "_" << maxrap << ".dat";

    ofstream outfile;
    outfile.open(fname.str().c_str());

    outfile << "#pt dNdpt" << endl;
	...
	for (int iy = 0; iy < neta; iy++) {
	    ...
	    for (int iphi = 0; iphi < nphi+1; iphi++) {
	        double phi = iphi*2*PI/nphi;
		philist[iphi] = phi;
	        dndydptdphi[iphi]=dydeta(eta_local, pt, m)*pt*particleList[j].dNdydptdphi[iy][ipt][iphi];
	    }

	    //cout << " Phi Start GSL Integral " << endl;	
	    gsl_interp_accel *phiacc = gsl_interp_accel_alloc ();
	    gsl_spline *phispline = gsl_spline_alloc (gsl_interp_linear, nphi+1);
	    gsl_spline_init (phispline, philist , dndydptdphi , nphi+1);

	    //cout << " Phi GSL Integral Initialized " << endl;	
    	    dndydpt[iy]=gsl_spline_eval_integ(phispline, minphi, maxphi, phiacc);
	    gsl_spline_free (phispline);
            gsl_interp_accel_free (phiacc);
        ...
        gsl_interp_accel *rapacc = gsl_interp_accel_alloc ();
        gsl_spline *rapspline = gsl_spline_alloc (gsl_interp_linear, neta);
        gsl_spline_init (rapspline, etalist, dndydpt , neta);
	    
	dndpt[ipt]=gsl_spline_eval_integ(rapspline, minrap, maxrap, rapacc);
        gsl_spline_free (rapspline);
        gsl_interp_accel_free (rapacc);

        outfile << pt << " " << dndpt[ipt] << endl;
...
// Compute dN/deta Med. Res. Dist.
void Freeze::get_dNdeta_dist(InitData *DATA, int number,
				  double minpt, double maxpt,
				  int yflag, double minrap, double maxrap) {
    ...
    double dndydptdphi[nphi+1] = {0};
    double dndydpt[npt] = {0};
    double dndeta[neta] = {0};

    // Set output file name
    stringstream fname;
    fname << "./outputs/MedRes_" << number << "_dNdeta_" << minpt << "_" << maxpt
          << "_eta_" << minrap << "_" << maxrap << ".dat";

    ofstream outfile;
    outfile.open(fname.str().c_str());

    outfile << "#eta dNdeta" << endl;
	...
	double rap_local = particleList[j].y[iy];
	    ...
	    //cout << " Phi GSL Integral Initialized " << endl;	
    	    dndydpt[ipt]=gsl_spline_eval_integ(phispline, minphi, maxphi, phiacc);
	    gsl_spline_free (phispline);
            gsl_interp_accel_free (phiacc);
        ...
        gsl_interp_accel *ptacc = gsl_interp_accel_alloc ();
        gsl_spline *ptspline = gsl_spline_alloc (gsl_interp_linear, npt);
        gsl_spline_init (ptspline, particleList[j].pt, dndydpt , npt);
	    
	dndeta[iy]=gsl_spline_eval_integ(ptspline, minpt, maxpt, ptacc);
        gsl_spline_free (ptspline);
        gsl_interp_accel_free (ptacc);

        outfile << eta_local << " " << dndeta[iy] << endl;
...
// Compute dN/dphi Med. Res. Dist.
void Freeze::get_dNdphi_dist(InitData *DATA, int number,
				  double minpt, double maxpt,
				  int yflag, double minrap, double maxrap) {
    ...
    double dndydptdphi[neta] = {0};
    double dndptdphi[npt] = {0};
    double dndphi[nphi+1] = {0};

    // Set output file name
    stringstream fname;
    fname << "./outputs/MedRes_" << number << "_dNdphi_" << minpt << "_" << maxpt
          << "_eta_" << minrap << "_" << maxrap << ".dat";

    ofstream outfile;
    outfile.open(fname.str().c_str());

    outfile << "#phi dNdphi" << endl;
    ...
    for (int iphi = 0; iphi < nphi+1; iphi++) {
	    ...
	    double pt = particleList[j].pt[ipt];	
	        ...
	        double rap_local = particleList[j].y[iy];
                ...
                dndydptdphi[iy]=dydeta(eta_local, pt, m)*pt*particleList[j].dNdydptdphi[iy][ipt][iphi];
            }

            gsl_interp_accel *rapacc = gsl_interp_accel_alloc ();
            gsl_spline *rapspline = gsl_spline_alloc (gsl_interp_linear, neta);
            gsl_spline_init (rapspline, etalist, dndydptdphi , neta);
	    
	    dndptdphi[ipt]=gsl_spline_eval_integ(rapspline, minrap, maxrap, rapacc);
            gsl_spline_free (rapspline);
            gsl_interp_accel_free (rapacc);	
    	...
    	gsl_interp_accel *ptacc = gsl_interp_accel_alloc ();
	gsl_spline *ptspline = gsl_spline_alloc (gsl_interp_linear, npt);
	gsl_spline_init (ptspline, particleList[j].pt , dndptdphi , npt);

    	dndphi[iphi]=gsl_spline_eval_integ(ptspline, minpt, maxpt, ptacc);
	gsl_spline_free (ptspline);
        gsl_interp_accel_free (ptacc);

        outfile << phi << " " << dndphi[iphi] << endl;
