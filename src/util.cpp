...
void LorentzBoost4Vector(double Tin[4], double *u, double Tout[4]){
  if(Tin[0] == 0. && Tin[1] == 0. && Tin[2] == 0. && Tin[3] ==0.){
    ...
    //For small beta2, do not change the vector:
    if(beta2 < 1.e-12){
	...
	for(int ia=1; ia<4; ia++){
      ...
      //With the Lorentz transformation matrix determined, the boosted vector can be calculated:
      for(int im=0; im<4; im++){
	for(int ia=0; ia<4; ia++){
	  Tout[im] += L[im][ia]*Tin[ia];
	}
      }
